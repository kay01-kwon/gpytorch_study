...
def draw_plot(model,
    ...
    dmean = torch.autograd.grad(mean.sum(), x_grid)[0]
    lb, ub = (mean - 2*var.sqrt(),
                  mean + 2*var.sqrt())

    mean_cpu = mean.cpu().view(-1)
    dmean_cpu = dmean.cpu().view(-1)
    ...
    with torch.no_grad():
        ...
        ax.plot(x_grid_cpu, dmean_cpu, linewidth=2,
            color='blue',label = 'Derivative')
...
def run_wiski():
    chunk_size = 40
    X, Y = get_data(num_rows=4*chunk_size - 1, fn='cos')
    ...
    # plt.show()
    plt.savefig('wiski_with_der.png', dpi=600)
